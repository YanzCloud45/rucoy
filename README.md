...
The GitHub Actions dependency validation no longer pipes `llvm-nm` or `llvm-ar` into early-exiting commands while `set -o pipefail` is active. This prevents false failures such as exit code 74 after `DobbyInstrument` is actually found.
